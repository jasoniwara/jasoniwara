      ...
      <div className="grid md:grid-cols-2 gap-8 md:gap-14 items-stretch border-b border-rule pb-14">
        <div className="flex flex-col justify-center order-2 md:order-1">
          ...
          <div className="flex items-center gap-4 kicker text-soft">
            ...
            {hero.readTime && (
              <>
                {story.date && <span aria-hidden="true">&mdash;</span>}
                <span>{hero.readTime}</span>
              </>
            )}
            <span aria-hidden="true">&mdash;</span>
            <Link href={story.href} className="text-press hover:underline">
              Read the full story
            </Link>
          </div>
        </div>
        ...
        <div className="order-1 md:order-2">
          <Link href={story.href} className="relative aspect-[4/3] w-full bg-panel border border-rule flex items-center justify-center overflow-hidden block">
            {story.image ? (
